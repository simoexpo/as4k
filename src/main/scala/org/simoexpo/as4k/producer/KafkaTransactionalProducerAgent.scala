...
class KafkaTransactionalProducerAgent[K, V](producerOption: KafkaProducerOption[K, V])(implicit actorSystem: ActorSystem,
  ...
  def produce(records: Seq[KRecord[K, V]], topic: String): Future[Seq[KRecord[K, V]]] =
    (actor ? ProduceRecords(records, topic)).map(_ => records).recoverWith {
      case ex: AskTimeoutException => Future.failed(KafkaTransactionalProducerTimeoutException(timeout, ex))
    }

  def produceAndCommit(record: KRecord[K, V], topic: String): Future[KRecord[K, V]] =
    (actor ? ProduceRecordsAndCommit(List(record), topic)).map(_ => record).recoverWith {
      case ex: AskTimeoutException => Future.failed(KafkaTransactionalProducerTimeoutException(timeout, ex))
    }

  def produceAndCommit(records: Seq[KRecord[K, V]], topic: String): Future[Seq[KRecord[K, V]]] =
    (actor ? ProduceRecordsAndCommit(records, topic)).map(_ => records).recoverWith {
      case ex: AskTimeoutException => Future.failed(KafkaTransactionalProducerTimeoutException(timeout, ex))
    }
}
